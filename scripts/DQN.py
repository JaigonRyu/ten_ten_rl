...
def obs_to_torch(obs):
    board = torch.as_tensor(obs["board"], dtype=torch.float32, device=DEVICE).unsqueeze(
        0
    )  # (1,10,10)
    hand_masks = torch.as_tensor(
...
class QNet(nn.Module):
    def __init__(self, n_actions=300):
        ...
        self.head = nn.Sequential(
            nn.Linear(board_out + hand_out, 512), nn.ReLU(), nn.Linear(512, n_actions)
        )

    def forward(self, board, hand_masks):
        if board.dim() == 3:
            board = board.unsqueeze(0)  # (B,1,10,10)
        if hand_masks.dim() == 3:
            hand_masks = hand_masks.unsqueeze(0)  # (B,3,5,5)
...
def batch_to_torch(batch):
    ...
    hands = torch.as_tensor(
        np.stack([b[1] for b in batch]), dtype=torch.float32, device=DEVICE
    )  # (B,3,5,5)
    acts = torch.as_tensor(
        [b[2] for b in batch], dtype=torch.int64, device=DEVICE
    )  # (B,)
    rews = torch.as_tensor(
        [b[3] for b in batch], dtype=torch.float32, device=DEVICE
    )  # (B,)
    nboards = torch.as_tensor(
        np.stack([b[4] for b in batch]), dtype=torch.float32, device=DEVICE
    )  # (B,10,10)
    nhands = torch.as_tensor(
        np.stack([b[5] for b in batch]), dtype=torch.float32, device=DEVICE
    )  # (B,3,5,5)
    dones = torch.as_tensor(
        [b[6] for b in batch], dtype=torch.float32, device=DEVICE
    )  # (B,)
    nmask = torch.tensor(
...
if __name__ == "__main__":
    ...
    while global_step < MAX_STEPS:  # for training purpose
        ...
        while not done and global_step < MAX_STEPS:  # for actual training
            ...
            if len(rb) >= start_learning and global_step % train_every == 0:
                if not USE_PER:
                    ...
                    loss, td_errors = dqn_update_per(
                        online, target, optimizer, batch, weights, gamma=gamma
                    )
        ...
        print(
            f"episode={ep:03d} return={ep_return:.1f} eps={epsilon:.3f} buffer={len(rb)}"
        )
        wandb.log(
            {
                ...
                "charts/mean_return_100": (
                    np.mean(returns_window) if returns_window else ep_return
                ),
            },
            step=global_step,
        )
